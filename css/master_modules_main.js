  ...
  $(document).on("click", ".blogLike", function () {
    ...
    $(this).transition({
      ...
      onComplete: function () {
        ...
        $.post(
          url + "blog/controller.php",
          { action: "like", id: id, type: like },
          function (json) {
            if (json.status === "success") {
              $.cookie("BLOG_voted", id, { expires: 120, path: "/" });
            }
          },
          "json"
        );
  ...
  $.extend(Comments.prototype, {
    ...
    bind: function () {
      ...
      $(this.element).on("click", "a.down, a.up", function () {
        ...
        var up = $(this).data("up");
        icon
          .removeClass("chevron up down")
          .addClass("check")
          .transition("fade in");
        $(this).removeClass("up down");
        $.post(
          base.settings.url + "controller.php",
          { action: "vote", type: type, id: id },
          function (json) {
            if (json.status === "success") {
              if (json.type === "down") {
      ...
      $(this.element).on("click", "a.replay", function () {
        ...
        var id = $(this).data("id");
        $.get(
          base.settings.url + "snippets/replyForm.tpl.php",
          { id: id },
          function (data) {
            var comment = $("#comment_" + id, base.element).children(
              ".content"
            );
            comment.append(data);
    ...
    submitComment: function (data) {
      var base = this;
      $.post(
        this.settings.url + "controller.php",
        data,
        function (json) {
          ...
          $("button[name=doReply], button[name=doComment]")
            .removeClass("loading")
            .prop("disabled", false);
        },
        "json"
...
$(function () {
    ...
    $.post(
      ...
      function (json) {
        if (json.status === "success") {
          ...
          $.notice(
            decodeURIComponent(
              "Ooops, there was an error selecting this item."
            ),
            { autoclose: 12000, type: "error", title: "Error" }
          );
        }
        setTimeout(function () {
  ...
  $("#digishop").on("click", ".digishopLike", function () {
    ...
    $(this).transition({
      ...
      onComplete: function () {
        $this.replaceWith('<i class="icon check"></i>');
        $.post(url + "digishop/controller.php", { action: "like", id: id });
      },
    });
  });
...
function Timeline(element, data, $, undefined) {
    ...
    if (timeline_data) {
      timeline_data.sort(function (a, b) {
        if (SELF._options.order === "desc") {
          return (
            parseInt(b.date.replace(/[^0-9]/g, ""), 10) -
            parseInt(a.date.replace(/[^0-9]/g, ""), 10)
          );
        } else {
          return (
            parseInt(a.date.replace(/[^0-9]/g, ""), 10) -
  ...
  this._createElement = function (element_data, position) {
    ...
    var element_box = $("<div>")
      .addClass("timeline_element_box")
      .appendTo(element);
    if (element_data.title) {
      var title = $("<div>")
        .addClass("timeline_title")
        .html(
          '<span class="timeline_title_label">' +
            element_data.title +
            '</span><span class="timeline_title_date">' +
            element_data.dateDisplay +
            "</span>"
        )
    ...
    switch (element_data.type) {
      case "iframe":
        var content = $("<div>")
          .addClass("wojo passive attached segment loading")
          .height(element_data.height)
          .appendTo(element_box);
        SELF._iframe_queue.push({ element: content, url: element_data.url });
        break;
      case "event":
        var box = $("<div>")
          .addClass("content")
          .html(element_data.content.body)
          .appendTo(element_box);
        if (element_data.content.venue.length) {
          $('<div class="wojo small divider"></div>').appendTo(box);
          var list =
            '<div class="item"> <i class="icon marker"></i><div class="middle aligned content"> ' +
            element_data.content.venue +
            " </div></div>";
          list +=
            '<div class="item"> <i class="icon phone"></i><div class="middle aligned content"> ' +
            element_data.content.phone +
            " </div></div>";
          list +=
            '<div class="item"> <i class="icon microphone"></i><div class="middle aligned content"> ' +
            element_data.content.person +
            " </div></div>";
          list +=
            '<div class="item"> <i class="icon email"></i><div class="middle aligned content"> ' +
            element_data.content.email +
            " </div></div>";
          $("<div>")
            .addClass("wojo small horizontal divided list")
            .html(list)
            .appendTo(box)
            .wrap('<div class="content-center"></div>');
        }
        break;
      case "gallery":
        if (element_data.images.length) {
          ...
          $('<div class="wojo carousel">' + image_html + "</div>").appendTo(
            element_box
          );
      ...
      case "blog_post":
        ...
        $(element_data.images).each(function (index, image_src) {
          if (element_data.images.length === 1) {
            ...
            html +=
              "" +
              '<div class="tmDimmer dimmable image">' +
              '<div class="wojo color dimmer">' +
              '<div class="content">' +
              '<div class="center"> <a href="' +
              image_src +
              '" class="tmLightbox inverted"><i class="icon large circular inverted url alt"></i></a> </div>' +
              "</div>" +
              "</div>" +
              '<img src="' +
              image_src +
        ...
        if (element_data.images.length > 1) {
          var $container = $("<div>")
            .addClass("wojo carousel")
            .html(html)
            .appendTo(element_box);
          $container.owlCarousel({ dots: true, nav: false, items: 1 });
        } else {
        ...
        if (element_data.readmore) {
          $("<div>")
            ...
            .html(
              '<a href="' +
                element_data.readmore +
                '">' +
                SELF._options.readmore_text +
                "</a>"
            )
            .appendTo(element_box);
        }
  ...
  this._createSeparator = function (time) {
    var separator = $("<div>")
      ...
      .attr("id", "timeline_date_separator_" + time)
      .html("<span>" + time + "</span>")
  ...
  this._render = function (timeline_data, is_append) {
    ...
    $(timeline_data).each(function (index, data) {
      ...
      if (is_append && index === 0) {
        ...
        if (
          last_element.length &&
          last_element.hasClass("timeline_element_left")
        ) {
          is_odd = false;
        }
      }
      ...
      var start_new = false;
      if (
        $.inArray(year, SELF._years) === -1 &&
        (SELF._options.separator === "year" || SELF._options.separator === null)
      ) {
        start_new = true;
        SELF._years.push(year);
      }
      if (
        $.inArray(month, SELF._months) === -1 &&
        (SELF._options.separator === "month" ||
          SELF._options.separator === "month_year")
      ) {
        start_new = true;
      ...
      if (start_new) {
        if (SELF._options.separator === "year") {
          ...
          if (SELF._months.length > 1 || SELF._options.first_separator) {
            var month_display =
              SELF._options.month_translation[
      ...
      setTimeout(function () {
        $(SELF._separators).each(function (index, separator) {
          if (SELF._use_css3) {
            ...
            separator
              .children("span")
              .animate({ opacity: 1, top: "50%" }, 300, function () {
                separator.addClass("animated");
              });
          }
        });
  ...
  this._loadMore = function () {
    ...
    setTimeout(function () {
      ...
      var new_data = SELF._original_data.slice(
        SELF._data.length,
        SELF._data.length + SELF._options.loadmore
      );
  ...
  this._display = function () {
    SELF._container = $("<div>").addClass(
      "timeline timeline_" + SELF._options.columnMode
    );
    if (!$.support.opacity) {
    ...
    if (
      SELF._options.loadmore &&
      SELF._options.max &&
      SELF._original_data.length &&
      SELF._original_data.length > this._options.max
    ) {
      SELF._loadmore = $("<div>")
        .addClass("wojo button timeline_loadmore")
        .text(SELF._options.loadmore_text)
        .appendTo(element);
  ...
  this._makeResponsive = function (responsive) {
    if (responsive) {
      if (!this._responsive) {
        ...
        SELF._container.removeClass(
          "timeline_left timeline_right timeline_dual"
        );
  ...
  this._processIframeQueue = function () {
    $(SELF._iframe_queue).each(function (index, queue) {
      queue.element
        .removeClass("loading")
        .html('<iframe frameborder="0" src="' + queue.url + '"></iframe>');
    });
  };
  ...
  this._handleKeyDown = function (e) {
    switch (parseInt(e.which, 10)) {
      ...
      case 37:
        if (
          SELF._lightbox.hasClass("loaded") &&
          SELF._lightbox.find(".navigation span.prev").is(":visible")
        ) {
      ...
      case 39:
        if (
          SELF._lightbox.hasClass("loaded") &&
          SELF._lightbox.find(".navigation span.next").is(":visible")
        ) {
  ...
  this._loadFacebook = function (callback) {
    ...
    var _initContent = function (facebook_data) {
      var content =
        '<div class="facebook_type_' +
        facebook_data.type +
        '">' +
        '<div class="facebook_left_column"><img class="facebook_profile" src="https://graph.facebook.com/' +
        facebook_data.from.id +
        '/picture?type=square" /></div>' +
        '<div class="facebook_right_column">';
      if (facebook_data.message) {
        content +=
          '<div class="facebook_content">' +
          facebook_data.message
            .trimString(SELF._facebook_message_length)
            .parseURL() +
          "</div>";
      }
    ...
    FB.init({ appId: SELF._options.facebookAppId, version: "v2.0" });
              ...
              function (response) {
                ...
                if (response.source) {
                  content +=
                    ...
                    '<a href="' +
                    facebook_data.link +
                    '" style="display:inline;"><img class="facebook_picture" align="left" src="' +
                    response.source +
                    '" /></a>' +
                    "</div>";
                }
              ...
              content +=
                ...
                (facebook_data.name
                  ? '<a href="' +
                    ...
                    "</a>"
                  : "") +
                (facebook_data.caption
                  ? '<div class="facebook_caption">' +
                    facebook_data.caption +
                    "</div>"
                  : "") +
                (facebook_data.description
                  ? '<div class="facebook_description">' +
                    facebook_data.description
                      .trimString(SELF._facebook_description_length)
                      .parseURL() +
                    "</div>"
                  : "") +
                "</div>" +
  ...
  this._loadTwitter = function (callback) {
    $.getJSON(
      "https://www.melonhtml5.com/api/?action=twittersearch&q=" +
        SELF._options.twitterSearchKey +
        "&callback=?",
      function (response) {
        ...
        $(response.statuses).each(function (index, tweet_data) {
          SELF._original_data.push({
            ...
            title:
              ...
              '" target="_blank" style="text-decoration:none;color:#AAAAAA;">' +
              tweet_data.user.screen_name +
              "</a>",
            content:
              '<div><img class="twitter_profile" align="left" src="' +
              tweet_data.user.profile_image_url +
              '" /></div>' +
              tweet_data.text.parseURL().parseHashtag(),
          });
        });
        if (callback !== undefined) {
          callback();
  ...
  this.display = function () {
    ...
    var has_facebook =
      SELF._options.facebookAppId &&
      SELF._options.facebookAccessToken &&
      SELF._options.facebookPageId;
  ...
  this.appendData = function (timeline_data) {
    var end_date = parseInt(
      SELF._data[SELF._data.length - 1].date.replace(/-/g, ""),
      10
    );
    var new_data = [];
    if (SELF._options.order === "desc") {
